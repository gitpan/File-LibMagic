# This Makefile is for the File::LibMagic extension to perl.
#
...
#     ABSTRACT_FROM => q[LibMagic.pm]
#     AUTHOR => q[Andreas Fitzner <fitzner@informatik.hu-berlin.de>]
#     DEFINE => q[]
#     INC => q[-I.]
#     LIBS => [q[-lmagic]]
#     NAME => q[File::LibMagic]
#     PREREQ_PM => {  }
#     VERSION_FROM => q[LibMagic.pm]
...
XS_FILES = LibMagic.xs
C_FILES  = LibMagic.c
O_FILES  = LibMagic.o
H_FILES  = 
MAN1PODS = 
MAN3PODS = LibMagic.pm
...
TO_INST_PM = LibMagic.pm

PM_TO_BLIB = LibMagic.pm \
	$(INST_LIB)/File/LibMagic.pm
...
# File::LibMagic might depend on some other libraries:
...
.c.i:
	i686-pc-linux-gnu-gcc -E -c $(PASTHRU_INC) $(INC) \
	$(CCFLAGS) $(OPTIMIZE) \
	$(PERLTYPE) $(MPOLLUTE) $(DEFINE_VERSION) \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  LibMagic.pm $(INST_MAN3DIR)/File::LibMagic.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                File-LibMagic' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.86' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            Perlwrapper for libmagic' >> META_new.yml
	$(NOECHO) $(ECHO) 'license:             ~' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:              ' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Andreas Fitzner <fitzner@informatik.hu-berlin.de>' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,86,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perlwrapper for libmagic</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Andreas Fitzner &lt;fitzner@informatik.hu-berlin.de&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  LibMagic.pm $(INST_LIB)/File/LibMagic.pm 
